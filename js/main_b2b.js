...
//lk notification b2b
(() => {
  let btnNotification = document.querySelector('.js_header_top_profile_hidden_item_b2b_notification');
  let popNotification = document.querySelector('.js_header_top_profile_hidden_b2b_notification');
  let closeNotification = document.querySelector('.js_header_top_profile_hidden_b2b_notification_header');
...
let compCharacter = document.querySelector('.comprasion_characteristics_body_slider_wrapper');
let shift = 0;
let diff = 0;
let flag = false;
...
(() => {
  ...
  for (let i = 0; i < checkBox.length; i++) {
    checkBox[i].addEventListener('click', function () {
      if (this.checked) {
        this.closest('.catalog_box_fl_item_body_el').classList.add('active');
      } else {
        this.closest('.catalog_box_fl_item_body_el').classList.remove('active');
      }
    })
  }
})();
...
//touchmove b2b lk
(() => {
  let box = document.querySelectorAll('.js_profile_box_orders_b2b_filter_wrap');
  let arr = [];
  ...
  if (box) {
    ...
    function difElemWidtch(paren) {
      arr = [];
      let elem = paren.querySelectorAll('.js_profile_box_orders_b2b_filter_item');
      for (let i = 0; i < elem.length; i++) {
        arr.push(elem[i].offsetWidth);
      }
      arr = arr.map(el => {
    ...
    function goLeft(paren) {
      difElemWidtch(paren);
      let item = paren.querySelector('.js_profile_box_orders_b2b_filter');
      let q = 0;
    ...
    let paren;
    function handleTouchMove(e) {
      paren = e.target.closest('.js_profile_box_orders_b2b_filter_wrap')
...
// tag b2b lk
(() => {
  let tag = document.querySelectorAll('.js_profile_box_orders_b2b_filter_item');
...
(() => {
  ...
  let userFix = document.querySelector('.js_header_b2b');
  let menuFix = document.querySelector('.fixed_menu_mobail');
  let closeFix = document.querySelector('.fixed_menu_mobail_header_close');
  let notific = document.querySelector('.js_fixed_menu_mobail_box_item_b2b_notific');
  let notificBack = document.querySelector('.js_fixed_menu_mobail_notification_back');
  let notificBox = document.querySelector('.js_level_to_b2b');
