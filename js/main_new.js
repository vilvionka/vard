...
function initBtnLike() {
  $('.catalog_box_cont_card_btn_like:not(.js_initialized)').on('click', function (e) {
  ...
  $('.catalog_box_cont_card_btn_like').addClass('js_initialized');
}
initBtnLike();
/* Favorites */
...
$(document).on("submit", "#addAddressForm", function (e) {
  ...
  var form = $(this);
  $.ajax({
    url: '/local/ajax/add_address.php', // URL отправки запроса
    type: "POST",
    data: form.serialize(),
    success: function (response) { // Если Данные отправлены успешно
      var result = $('<div />').append(response).find('.profile_address').html();
      $('.profile_address').html(result);
...
$(document).on("submit", ".editProfileForm", function (e) {
  ...
  var form = $(this);
  $.ajax({
    url: '/local/ajax/edit_address.php', // URL отправки запроса
    type: "POST",
    data: form.serialize(),
    success: function (response) { // Если Данные отправлены успешно
...
$(document).on("click", ".js_profile_address_body_box_delete", function (e) {
  elem = $(this);
  id = $(this).data('id');
  var param = "id=" + id;
  $.ajax({
    url: '/local/ajax/remove_profile.php', // URL отправки запроса
    type: "GET",
    data: param,
    success: function (response) { // Если Данные отправлены успешно
      if (response == "OK") {
        elem.closest('.profile_address_body').remove();
      }
...
$('.profile_block_edit_body_flex_item input').on('change', function (e) {
  key = $(this).attr('name');
  val = $(this).val();
  var param = key + "=" + val;
  $.ajax({
    url: '/local/ajax/save_profile.php', // URL отправки запроса
    type: "GET",
    data: param,
    success: function (response) { // Если Данные отправлены успешно
      var result = $('<div />').append(response).find('.js_profile_block').html();
      $('.js_profile_block').html(result);
...
$(document).ready(() => {
  $('.favorites_box_btn').on('click', function (e) {
    let ids = e.target.getAttribute('data-items-ids').split(',');
    ...
    $.ajax({
      url: '/local/templates/WBE70/ajax/add2basket.php',
      type: "POST",
      data: param,
      success: function (response) { // Если Данные отправлены успешно
  ...
  $.ajax({
    ...
    success: function (response) { // Если Данные отправлены успешно
      ...
      if (result == 1) {
        ...
        $('.catalog_box_cont_card_btn_like[data-item="' + id + '"]').addClass('active');
        var wishCount = parseInt($('#want .header_bottom_comparison_num').html()) + 1;
        $('#want .header_bottom_comparison_num').html(wishCount); // Визуально меняем количество у иконки
      }
      if (result == 2) {


        $('.catalog_box_cont_card_btn_like[data-item="' + id + '"]').removeClass('active');
        var wishCount = parseInt($('#want .header_bottom_comparison_num').html()) - 1;
        $('#want .header_bottom_comparison_num').html(wishCount); // Визуально меняем количество у иконки
      }
...
/* Compare list */
/*tabs*/
$('.comprasion_filter_box_item').on('click', function (e) {
  let compareBox = document.querySelector('section.comprasion');
  let favoriteBox = document.querySelector('section.favorites');

  if (favoriteBox) {
    let btn = e.currentTarget;
    let title = btn.querySelector('p').textContent;
    $('.catalog_headling_tittle').text(title);

    let sectionId = btn.getAttribute('data-section-id');
    $('.catalog_box_cont_card').removeClass('active').addClass('hidden').hide();
    $('.favorites_box_card').find('[data-section-id="' + sectionId + '"]').removeClass('hidden').addClass('active').show();
  }

  if (compareBox) {
    let sectionEl = '.' + $(this).attr('for');
    $('.comprasion_box').css('display', 'none');
    $('.comprasion_characteristics').css('display', 'none');
    $('.comprasion_box' + sectionEl).css('display', 'flex');
    $('.comprasion_characteristics' + sectionEl).css('display', 'block');
  }
});

/*//tabs*/

function initBtnCompare() {
  $('.catalog_box_cont_card_btn_comparison:not(.js_initialized)').on("click", function (e) {
    var favorID = $(this).attr('data-item');
  ...
  $('.catalog_box_cont_card_btn_comparison').addClass('js_initialized');
}
initBtnCompare();

$('.comprasion_box_slider_item_top_btn_clear').on('click', function (e) {
  var favorID = $(this).attr('data-item');
  var doAction = 'DELETE_FROM_COMPARE_LIST'; //'delete';

  addCompare(favorID, doAction);
});

$('.comprasion_box_first_clear').on('click', function (e) {
  var Ids = $(this).attr('data-ids');
  var doAction = 'DELETE_FROM_COMPARE_LIST'; //'delete';
  var IblockId = $(this).attr('data-iblock-id');
  addCompare(Ids, doAction, IblockId);
});
/* //Compare list */

/* Список сравнения */
function addCompare(id, action, IblockId = false) {
  var param = 'id=' + id + "&action=" + action;
  if (IblockId) {
    param += '&iblock_id=' + IblockId;
  }
  $.ajax({
    url: '/local/templates/WBE70/ajax/list_compare.php', // URL отправки запроса list_compare
    type: "GET",
    dataType: "html",
    data: param,
    success: function (response) { // Если Данные отправлены успешно
      ...
      if (result['MESSAGE'] == 'Товар добавлен в список сравнения') { //result == 1
        // Если всё чётко, то выполняем действия, которые показывают, что данные отправлены
        $('.catalog_box_cont_card_btn_comparison[data-item="' + id + '"]').addClass('active');
        var compareCount = parseInt($('#compare .header_bottom_comparison_num').html()) + 1;
        $('#compare .header_bottom_comparison_num').html(compareCount); // Визуально меняем количество у иконки
      }
      if (result['MESSAGE'] == 'Товар удален из списка сравнения') { //result == 2
        $('.catalog_box_cont_card_btn_comparison[data-item="' + id + '"]').removeClass('active');
        var compareCount = parseInt($('#compare .header_bottom_comparison_num').html()) - 1;
        $('#compare .header_bottom_comparison_num').html(compareCount); // Визуально меняем количество у иконки
      }
      //if(result['COUNT'] === 0 || result['MESSAGE'] == 'Товар не найден'){
      //  location.reload();
      //}
    },
    error: function (jqXHR, textStatus, errorThrown) { // Ошибка
...
// catalog menu
(() => {
  let catMenuItem = document.querySelectorAll('.header_catalog_box_menu a');
...
(() => {
  ...
  if (clearButton) {
    ...
    clearButton.addEventListener('click', function () {
      window.location = '/catalog/filter/clear/apply/';
...
// catalog card more
/*
* перенесено в компонент WBE70:catalog.section
* (local\components\WBE70\catalog\templates\.default\bitrix\catalog.section\.default\script.js)
*/
...
$(document).ready(function () {
  $('.js_notification').each(function () {
    var notification = $(this),
      cookieName = notification.data('notification'),
    ...
    if (cookieName) {
      ...
      notification.find('.js_notification_box_btn').on("click", function () {
        Cookies.set(cookieName, "accepted", { expires: expires });
  ...
  if (box) {
    ...
    for (let i = 0; i < menuItem.length; i++) {
      menuItem[i].addEventListener('click', function () {
        ...
        let sortMethod = this.getAttribute('data-sort-method');
        sorting(sort, sortMethod);
      })
    }

  }
  function sorting(sort, sortMethod) {
    if (sort == 'price') {
      let wrapper = $('.catalog_box_cont_card').parent();

      if (sortMethod == 'asc') {
        wrapper.find('.catalog_box_cont_card').sort(function (a, b) {
...
/* перенёс в local/components/repair/form/templates/.default/script.js
// repair one
...
var serialNumberUserValue = null;
...
$(document).on("click", ".js_profile_head_body_tittle_edit", function () {
  $('.js_profile_block_edit').addClass('active');
  $('.js_profile_block').addClass('remove');
});

$(document).on("click", ".js_profile_block_edit_head_left_back", function () {
  $('.js_profile_block_edit').removeClass('active');
  $('.js_profile_block').removeClass('remove');
});
...
function initBuyClick() {
  ...
  if (buyClick) {
    for (let i = 0; i < buyClick.length; i++) {
      if (buyClick[i].classList.contains('js_initialized')) {
        continue;
      }
      ...
      buyClick[i].classList.add('js_initialized');
    }
  }
  if (closeBuy) {
...
function initBacketClick() {
  ...
  if (backetClick) {
    for (let i = 0; i < backetClick.length; i++) {
      if (backetClick[i].classList.contains('js_initialized')) {
        continue;
      }
      backetClick[i].addEventListener('click', function () {
        ...
        console.log(this.dataset, backetClick[i]);
        ...
        let nextSblg = this.nextElementSibling;
        if (nextSblg) {
          nextSblg.classList.add('active');
        }
        document.body.classList.add('noScroll');
        addToCardForm(backetClick[i]);
        setTimeout(timePopBacket, 2500);
      });
      backetClick[i].classList.add('js_initialized');
    }
  }
  if (closeBacket) {
...
function addToCardForm(form, quantity = 1) {
  let param = {
...
function initCompliteddCalc() {
  ...
  for (let i = 0; i < cmplitedMinus.length; i++) {
    if (cmplitedMinus[i].classList.contains('js_initialized')) {
      continue;
    }
    cmplitedMinus[i].addEventListener('click', function () {
      let parentCmplitedNum = cmplitedMinus[i].closest('.js_backet_parent');
      let cartNem = parentCmplitedNum.querySelector('.js_backet_click');
      let num_prod = Number(cmplitedNum[i].innerHTML) - 1;
      cmplitedNum[i].innerHTML = num_prod;
      if (Number(cmplitedNum[i].innerHTML) < 1) {
        cmplitedNum[i].innerHTML = num_prod = 1;
      }
      addToCardForm(cartNem, num_prod)
    });
    cmplitedMinus[i].classList.add('js_initialized');
  }

  for (let i = 0; i < cmplitedPlus.length; i++) {
    if (cmplitedPlus[i].classList.contains('js_initialized')) {
      continue;
    }
    ...
    cmplitedPlus[i].classList.add('js_initialized');
  }
}
  ...
  if (inpSubmit) {
    inpSubmit.addEventListener('click', function (e) {
      ...
      function functionAjax() {
        // Формирование данных для отправки на сервер Битрикс24
        let formData = new FormData();
        formData.append('form_title', 'Подписались на рассылку');
        formData.append('email', inpEmail.value);


        // Отправка AJAX-запроса на сервер Битрикс24
        fetch('integration/b24popup.php', {
          method: 'POST',
          body: formData
        })
      ...
      if (validE && inpCheck.checked) {
        ...
        functionAjax();
        document.querySelector('.subscription_input_box input').value = ''
...
//проверка индекса добавление адресса в личном кабинете
(() => {
  ...
  if (inpSubmit) {
    inpSubmit.addEventListener('click', function (e) {
      ...
      if (flagHome && flagCity && flagIndex && flagStreet) {
        e.preventDefault();
        e.stopPropagation();
        let pop = document.querySelector('.js_pop_add_address');
        if(pop){
...
(() => {
  ...
  // Функция для выполнения AJAX-запроса на сервер Битрикс24
  function functionAjax() {
    // Формирование данных для отправки на сервер Битрикс24
    let formData = new FormData();
    formData.append('form_title', 'Заказ обратного звонка');
    formData.append('first_name', inpName.value);
    formData.append('first_name', inpName.value);
    formData.append('phone', inpPhone.value);
  ...
  if (inpSubmit) {
    inpSubmit.addEventListener('click', function (e) {
      ...
      if (inpName.value.length > 3) {
        ...
        outputName = 'Недостаточное количество символов в поле имя!';
        flagName = false;
      }
      ...
      if (flagName && flagPhone && flagCheck) {
        e.preventDefault();
        e.stopPropagation();
        functionAjax(); // Вызываем функцию для выполнения AJAX-запроса
...
(() => {
  const form = document.querySelector(".js_pop_buy form");
  ...
  const product = document.querySelector(".pop_buy_product_subscription > span");
  const price = document.querySelector(".pop_buy_product_subscription > p");
  const id = document.querySelector(".pop_buy_product_subscription > i");
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (inpName) {
        ...
        function functionAjax() {
          // Формирование данных для отправки на сервер Битрикс24
          let formData = new FormData();
          formData.append('form_title', 'Покупка в 1 клик');
          formData.append('first_name', inpName.value);
          formData.append('phone', inpPhone.value);

          formData.append("price", price.innerHTML.replace(/[\s₽]/g, ''));
          formData.append("product", product.innerHTML + " [" + id.innerHTML + "]");
          // console.debug(formData); return;

          // Отправка AJAX-запроса на сервер Битрикс24
          fetch('integration/buy_one_click.php', {
            method: 'POST',
            body: formData
          })
        ...
        if (flagName && flagPhone) {
          functionAjax()
          document.querySelector('.js_pop_buy').classList.remove('active');
          document.body.classList.remove('noScroll');
        }
...
(() => {
  ...
  let costPromo = document.querySelector('.js_backet_bar_box_item_product_sale span');
  ...
  for (let i = 0; i < choiseProduct.length; i++) {
    choiseProduct[i].addEventListener('click', (e) => {
      calcBacket(), delayProdBasket(e)
    });
  }
  for (let i = 0; i < choiseProduct.length; i++) {
  ...
  function delayProdBasket(e) {
    let id = e.target.value;
    console.log(id);
    if (id) {
      $.ajax({
        url: '/local/templates/WBE70/ajax/changeFieldProdBasket.php', // URL отправки запроса
        type: "POST",
        data: {
          'PRODUCT_ID': id,
          'CHANGE_DELAY': true
  ...
  function popDeleteProduct(paren) {
    ...
    let itemId = paren.querySelector('.backet_body_box_product_three_delete').getAttribute('data-item-id');
    let quantity = paren.querySelector('.backet_num').textContent;
    pop.querySelector('a').setAttribute('onclick', 'backToCart(' + itemId + ',' + quantity + ')');
  ...
  function totalBcket() {
    if (costProduct) {
      ...
      costTotal.innerHTML = costTotal.innerHTML.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ' + '₽';
      // }
    }
    mobPriceBacket();
  ...
  function mobPriceBacket() {
    ...
    if (mobPrice) {
      if (a) {
        mobPrice.innerHTML = costTotal.text;
  ...
  function calcBacket() {
    ...
    if (choiseProduct) {
      for (let i = 0; i < choiseProduct.length; i++) {
        checkoutButton[i] = choiseProduct[i].checked ? true : false;
        if (choiseProduct[i].checked && choiseProduct[i].closest('.backet_body_box_product')) {
          ...
          let box = choiseProduct[i].closest('.backet_body_box');
          box.classList.add('active');
          let num = paren.querySelector('.backet_body_box_product_too_total .basket_item_price').innerText.replace(/[^+\d]/g, '');
          globalSum = globalSum + Number(num);
          let numOld = paren.querySelector('.backet_body_box_product_too_total .basket_item_price_old').innerText.replace(/[^+\d]/g, '');
          globalSumOld = globalSumOld + Number(numOld);
          let quantity = paren.querySelector('.backet_num').innerHTML;
        ...
        if (choiseProduct[i].checked == false && choiseProduct[i].closest('.backet_body_box_product')) {
          ...
          if (paren.querySelector('.js_backet_body_box_add_head')) {
        ...
        let discountSum = globalSumOld - globalSum;
        costProduct.innerText = globalSumOld.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ' + '₽';
        costPromo.innerText = discountSum.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ' + '₽';
        productNum.innerHTML = totalNum + ' ' + 'шт';
      }
    }

    if (checkoutBtn && checkoutButton.includes(true)) {
  ...
  function numPlus(e) {
    let head = e.target.closest('.backet_body_box')
    let prodId = head.querySelector('input[name="PRODUCTS[]"]').value;
    let detect = head.querySelector('.js_backet_body_box_add_head');
    ...
    diffTotal(paren, numDiff, '.basket_item_price');
    diffTotal(paren, numDiff, '.basket_item_price_old');
    calcBacket();
    totalBcket();
    noteAcc(head, detect);
    changeQuantity(prodId, numDiff);
  }

  function diffTotal(paren, numDiff, selector) {
    let price = paren.previousElementSibling.querySelector(selector).innerText.replace(/[^+\d]/g, '');
    let priceCost = Number(price) * numDiff;
    let priceCostSpace = priceCost.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + ' ' + '₽';
    paren.nextElementSibling.querySelector(selector).innerHTML = priceCostSpace;
  }
  ...
  var promoContainer = $('.js_backet_bar_box_promocode');
  $('.basket_promocode_button').on('click', function () {
    var coupon = $('#basket_promocode').val();
    if (!$(this).hasClass('processing')) {
      $(this).addClass('processing');

      $.ajax({
        type: 'POST',
        url: basketAjaxPath + 'coupon.php',
        data: {
          'coupon': coupon,
        ...
        success: function (data) {
          if (data.success) {
            var couponInfo = data.success.coupon,
              basketInfo = data.success.basket;

            promoContainer.find('.promo-status').text(couponInfo.STATUS);

            $.each(basketInfo, function (id, item) {
              var target = $('.backet_body_box[data-id="' + id + '"]'),
                priceOld = target.find('.backet_body_box_product_too_cost .basket_item_price_old'),
                priceOldTotal = target.find('.backet_body_box_product_too_total .basket_item_price_old'),
                price = target.find('.backet_body_box_product_too_cost .basket_item_price'),
                priceTotal = target.find('.backet_body_box_product_too_total .basket_item_price');

              if (target.length) {
                if (item.DISCOUNT > 0) {
                  priceOld.addClass('active');
                  priceOldTotal.addClass('active');
                } else {
                  priceOld.removeClass('active');
                  priceOldTotal.removeClass('active');
                }
                priceOld.text(item.BASE_PRICE_FORMATTED);
                price.text(item.PRICE_FORMATTED);

                priceTotal.text(item.PRICE_ALL_FORMATTED);
                priceOldTotal.text(item.BASE_PRICE_ALL_FORMATTED);
              }
            });
...
function backToCart(id, quantity) {
  if (id && quantity) {
    $.ajax({
      url: '/local/templates/WBE70/components/bitrix/sale.basket.basket/.default/ajax/backToCart.php', // URL отправки запроса
      type: "POST",
      data: {
        'PRODUCT_ID': id,
        'QUANTITY': quantity
...
// more review
function sliceText() {
  var reviewsSalon = document.querySelectorAll('.card_review_box_item_wrapper_text.comment p');
  var reviewsBtn = document.querySelectorAll('.card_review_box_item_wrapper_text i');
  var arrReview = [];
  if (reviewsSalon) {
    for (let i = 0; i < reviewsSalon.length; i++) {
      arrReview.push(reviewsSalon[i].innerText);
      let dl = reviewsSalon[i].innerText.length;
      if (dl > 350 && window.innerWidth >= 900) {
        reviewsSalon[i].innerText = reviewsSalon[i].innerText.slice(0, 350) + '... ';
        reviewsSalon[i].closest('.card_review_box_item_wrapper_text').querySelector('i').classList.add('active');
      }
    }
    if (document.documentElement.clientWidth <= 900) {
      for (let i = 0; i < reviewsSalon.length; i++) {
        reviewsSalon[i].innerText = arrReview[i];
      }
    }
    for (let i = 0; i < reviewsBtn.length; i++) {
      reviewsBtn[i].addEventListener('click', function () {
        if (this.classList.contains('active')) {
          this.closest('.card_review_box_item_wrapper_text').querySelector('p').innerText = arrReview[i];
          reviewsBtn[i].classList.remove('active');
          reviewsBtn[i].classList.add('collapse');
          reviewsBtn[i].innerText = 'свернуть';
          return;
        }
        if (this.classList.contains('collapse')) {
          this.closest('.card_review_box_item_wrapper_text').querySelector('p').innerText = this.closest('.card_review_box_item_wrapper_text').querySelector('p').innerText.slice(0, 350) + '... ';
          reviewsBtn[i].classList.remove('collapse');
          reviewsBtn[i].classList.add('active');
          reviewsBtn[i].innerText = 'Читать полностью';
          return;
        }
      })
    }
